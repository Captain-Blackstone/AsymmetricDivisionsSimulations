...
class Simulation:
    ...
    def check_convergence_v2(self):
        ...
        if self.history.times[-1] > critical_period:
            ...
            if len(set(np.round(np.array(self.history.harshnesses)[ii], 2))) == 1 and len(
                    np.round(np.array(self.history.harshnesses)[ii], 2)) > 1:
                ...
                if len(minima) >= 2 and len(maxima) >= 2:  # If there were more than two minima and maxima
                    ...
                    if self.convergence_estimate_first_order is not None and \
                            ...
                            round(self.convergence_estimate_first_order[0], 2) == round(estimate, 2):
                        self.converged = True
                ...
                if len(smoothed) > 5:
                    index_array = np.where(np.round(smoothed, 2) != np.round(smoothed, 2)[-1])[0]
    ...
    def save(self):
        ...
        with open(
            ...
            if self.population_sizes[-1] < 1 and all(
                ...
                self.harshnesses = [self.population_sizes[0], self.population_sizes[-1]]
...
if __name__ == "__main__":
    ...
    if simulation.converged == False:
        ...
        tried_harshnesses = [0]
        current_harshness = 0
        while abs(current_harshness - target_harshness) > 0.01:
            ...
            simulation = Simulation(params=test_parameters, mode=args.mode,
                                    ...
                                    discretization_volume=args.discretization_volume,
                                    discretization_damage=args.discretization_damage)
            current_harshness = simulation.run(1000000)
